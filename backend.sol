...
contract EventTicket is ERC721, Ownable, IERC2981 {
    ...
    function mintTickets(address recipient, uint256 quantity) public payable returns (uint256[] memory) {
        require(remainingSupply >= quantity, "Not enough tickets");
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            uint256 newTicketId = _ticketIds.current();
            _safeMint(recipient, newTicketId);
            tickets[newTicketId] = Ticket({eventTimestamp: eventTimestamp, isUsed: false});
            lastTransferTimestamp[newTicketId] = block.timestamp;
            ticketIds[i] = newTicketId;
            emit TicketMinted(recipient, newTicketId);
        }
        unchecked { remainingSupply -= quantity; }
        ...
        returns (address)
    {
        ...
        if (from != address(0)) {
            require(!tickets[tokenId].isUsed && block.timestamp < tickets[tokenId].eventTimestamp, "Invalid ticket");
            if (to.code.length > 0) {
